...
export const Accordion: React.FC<AccordionProps> = ({ renderTitle, children, defaultOpen = false, ...props }) => {
  ...
  return (
    <div className="py-3 w-full" {...props}>
      <FlexBox className="cursor-pointer justify-between items-center" onClick={toggleAccordion}>
        <div>{renderTitle()}</div>
        <div className={`transform transition-transform ${isOpen ? 'rotate-0' : 'rotate-180'}`}>
