...
export const AISummaryBox = ({ goalId }: { goalId: string }) => {
  const router = useRouter();
  const [goalRetrospect, setGoalRetrospect] = useState<string>('');
  const { showToast } = useToast();

  const patchGoalRetrospect = async () => {
    try {
      await patchCompletedGoalRetrospect(goalId, goalRetrospect);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/** 나의 회고 */}
      <div className="flex flex-col gap-4 w-full bg-[rgba(58,238,73,0.04)] border-2 border-line-normal py-5 px-6 rounded-lg mt-4">
        ...
        <FlexBox direction="col" className="gap-2 w-full">
          ...
          <div className="w-[106px] flex items-end justify-end">
            <Button onClick={patchGoalRetrospect} variant="secondary" size="sm" text="작성 완료" />
          </div>
        </FlexBox>
      </div>
    </div>
