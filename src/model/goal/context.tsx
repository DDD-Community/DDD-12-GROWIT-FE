...
export function GoalProvider({ children }: { children: React.ReactNode }) {
  ...
  // 서버에서 prerendering 시점에 데이터를 불러올 수 없기 때문에 클라이언트 사이드에서만 쿼리 실행
  const progressGoalQuery =
  ...
  useEffect(() => {
    // 최초에 현재 선택된 목표값 초기화
    if (progressGoalQuery && progressGoalQuery.data) {
      setCurrentGoal(progressGoalQuery.data[0] || null);
    }
  }, [progressGoalQuery]);

  const value: GoalContextType = {
    progressGoals: progressGoalQuery ? progressGoalQuery.data : [],
    currentGoal,
    setCurrentGoal,
  };
